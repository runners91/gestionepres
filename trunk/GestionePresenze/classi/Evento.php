    ...
    */
    ...
    static function contaGiorniMese($data,$causale,$utente) {
        ...
        $rs = Database::getInstance()->eseguiQuery("SELECT id_evento,data_da,data_a,durata
                                                    ...
                                                    FROM_UNIXTIME(data_da,'%c.%Y') <= ? AND
                                                    FROM_UNIXTIME(data_a,'%c.%Y') >= ? AND
                                                    fk_dipendente = ?",array($causale,$mese,$mese,$utente));
        $giorni = 0;
        while(!$rs->EOF) {
     ...
     * @return la durata in giorni dell'evento
     */
    static function contaGiorniMesePerFiliali($data,$causale,$filiale) {
        $mese = date("n.Y",$data);
        $rs = Database::getInstance()->eseguiQuery("SELECT id_evento,data_da,data_a,durata
                                                    FROM eventi
                                                    WHERE fk_causale = ? AND
                                                    FROM_UNIXTIME(data_da,'%c.%Y') <= ? AND
                                                    FROM_UNIXTIME(data_a,'%c.%Y') >= ? AND
                                                    fk_dipendente in (SELECT id_dipendente 
                                                                      FROM dipendenti 
                                                                      WHERE fk_filiale = ? )",array($causale,$mese,$mese,$filiale));
        $giorni = 0;
        while(!$rs->EOF) {
            $incremento = $rs->fields["durata"]=="G"?1:0.5;
            $d = date("j",$rs->fields["data_da"]);
            while(true){
                ...
                if(date("N",$data)<=5 && date("n.Y",$data) == $mese) {
                    $sql = "SELECT durata
                            ...
                            AND (fs.data = ? OR FROM_UNIXTIME(fs.data,'%d.%c') = ? AND fs.ricorsivo = 1)
                            AND f.id_filiale = ?;";
                    $rs2 = Database::getInstance()->eseguiQuery($sql,array($data,date("j.n",$data) ,$filiale));
                    if($rs2->rowCount()==0)
                        $giorni += $incremento;
                    else if($rs2->fields["durata"]!="G")
                        $giorni += 0.5;
